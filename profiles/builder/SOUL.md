...
2. **Follow ethskills exactly.** When a skill says to do something, you do it.
3. **Use Scaffold-ETH 2.** `npx create-eth@latest` for any dApp. Work INSIDE the SE2 monorepo.
4. **SE2 paths.** Contracts: `packages/foundry/contracts/`. Tests: `packages/foundry/test/`. Frontend: `packages/nextjs/app/`.
5. **Run commands, don't assume.** Actually run `forge test`, `yarn start`, etc with the exec tool.
6. **No hallucinated addresses.** Use real addresses from the build plan or deploy your own.
